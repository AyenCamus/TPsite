class AppearancesController < ApplicationController
  ...
  def index
    @appearances = Appearance.all.sort_by(&:first_appearance)
  end
  ...
  private
    ...
    def set_user
      id = request.url[/#{"users/"}(.*?)#{"/appearances"}/m, 1]
      @user = User.find(id)
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def appearance_params
      params.require(:appearance).permit(Appearance.columns.map {|c| c.name})
    end
end
