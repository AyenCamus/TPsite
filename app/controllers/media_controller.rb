class MediaController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @medium.save
        format.html { redirect_to @user, notice: 'Medium was successfully created.' }
  ...
  private
    ...
    def set_user
      id = request.url[/#{"users/"}(.*?)#{"/media"}/m, 1]
      @user = User.find(id)
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def medium_params
      params.require(:medium).permit(Medium.columns.map {|c| c.name})
    end
end
